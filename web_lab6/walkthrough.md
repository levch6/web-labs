...
- **Backend Organization**: Moved backend files to `server/` directory.
  - `server/server.php`: Handles API requests.
  - `server/data.json`: Stores tab data.
- **JavaScript Refactoring**: Moved inline JavaScript to external files in the `js/` directory.
...
- Fetches data from `server/server.php` on load and every 3 seconds (auto-update).
